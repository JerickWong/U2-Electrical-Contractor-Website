...
function Accounts() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (user.type === "Manager") setRedirect("Manager");
      else if (user.type === "Employee") setRedirect("Employee");
  ...
  useEffect(() => {
    if (username === "" || password === "") setComplete(false);
    else setComplete(true);
  }, [username, password]);

  const checkAdmin = () => {
    if (redirect === "Manager") return <Redirect to="/AdminMts" />;
    else if (redirect === "Employee") return <Redirect to="/Mts" />;
  };
  ...
  const handleDelete = async () => {
    if (deleteAccount.type === "Admin") alert("Cannot delete Admin account");
    else {
      setOpen(true);
      setLoading(true);
      setAction("Delete");
      try {
        await (
          await users.deleteUser(deleteAccount._id)
        ).data.success;
        setTimeout(() => {
          setLoading(false);
          setSuccess(true);
                  ...
                  <DialogContent dividers>
                    <div className="modalAcc">
                      ...
                      <FormGroup>
                        ...
                        />
                        <FormHelperText className={classes.modalFields}>
                          Leave blank to have password unchanged
                        </FormHelperText>
                      </FormGroup>
                      <FormGroup>
                        <InputLabel className={classes.modalFields}>
