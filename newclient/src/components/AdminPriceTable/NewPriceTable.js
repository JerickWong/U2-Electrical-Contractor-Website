          ...
          category.items.map((cat, index) => {
            return (
              <tr key={cat._id} style={{backgroundColor: props.isAdding && ''}}>
                ...
                <td>{isEditing[index] ? <Form.Control onChange={e => handleChange(e, index) } name="list_price" min={0} type="number" size="sm" defaultValue={cat.list_price}/> : Number(parseFloat(cat.list_price)).toLocaleString('en-US')}</td>
                ...
                <td>{isEditing[index] ? <Form.Control onChange={e => handleChange(e, index) } name="net_price" min={0} type="number" size="sm" defaultValue={cat.net_price}/> : Number(parseFloat(cat.net_price)).toLocaleString('en-US')}</td>
